    ...
    if file and '.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in allowed_ext:
        ...
        try:
            if file_type == 'students':
                ...
                data = None
            return jsonify({'message': 'File uploaded successfully', 'data': standardize_keys(data) if file_type=='students' else data})
...
def generate_seating():
    ...
    hall_file = data.get('hall_file') or get_latest_files('halls_')[0]
    schedule_file = data.get('schedule_file') or get_latest_files('schedule_')[0]
    exam_type = data.get('exam_type', 'Internal')
    session_type = data.get('session_type', 'FN')
    ...
    arrangement_df = generate_seating_arrangement(all_students, halls, schedule, exam_type, session_type)
    output_file = export_seating_arrangement(arrangement_df)

    return jsonify({'message': 'Seating arrangement generated', 'file': output_file, 'data': arrangement_df.to_dict('records')})
# -----------------------------
# Slot Report & Validation
# -----------------------------
...
def slot_report():
    try:
        ...
        return jsonify({'message': 'Slot report generated', 'data': report.to_dict('records'), 'slot_mapping': DEPT_SLOT_MAPPING})
    except Exception as e:
...
def validate_assignments():
    try:
        ...
        return jsonify({'message': 'Validation completed', 'invalid_assignments': invalid_assignments, 'total_students': len(all_students), 'invalid_count': len(invalid_assignments)})
    except Exception as e:
...
def get_latest_files(prefix):
    files = [os.path.join(app.config['UPLOAD_FOLDER'], f) for f in os.listdir(app.config['UPLOAD_FOLDER']) if f.startswith(prefix)]
